...
and use it to extract IPA strings from a file, a list of MWPage objects, or a string.
"""
...
class MWIPAExtractor(object):
    ...
    def extract_from_file(self, dump_file_path):
        return self.extract_from_pages(MWParser(full_parsing=False).parse_file(dump_file_path).pages)
    ...
    def extract_from_pages(self, pages):
        ...
        for p in pages:
            ...
            if (len(p.title) > 0) and (ipa is not None) and (len(ipa) > 0):
                mwdata.append(MWData(True, p.id, p.title, ipa))
                pages_with_ipa += 1
            else:
                mwdata.append(MWData(False, p.id, p.title, None))
        return MWExtractionInfo(mwdata=mwdata, pages_total=len(pages), pages_with_ipa=pages_with_ipa)
